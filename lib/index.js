...
class TCRListings {
  constructor(applicationLogs, nonApplicationLogs, user) {
    this.user = user
    this.applicationLogs = applicationLogs
    this.nonApplicationLogs = nonApplicationLogs
  ...
  async getListings(localListings) {
    const applicationListings = await this.batchCreateListings(this.applicationLogs, [])
  ...
  async createListing(logData, txData) {
    let { listingHash, listingID, data, appEndDate, applicant, _eventName } = logData
    ...
    return {
      ...
      appEndDate: appEndDate.toString(),
      latestEvent: _eventName,  // Recent event name
      ...
      if (applications.length > 0) {
        ...
        const chunkListings = await Promise.all(
          chunkApplications.map(application =>
            this.createListing(application.logData, application.txData)
          )
        )
  ...
  changeMatchListingWithLog(matchListing, logData, txData, eventName) {
    ...
    if (txData.get('blockTimestamp') < matchListing.getIn(['latestBlockTxn', 'blockTimestamp'])) {
    ...
    const olderMatch = matchListing
      .set('latestBlockTxn', txData)
      .set('latestEvent', eventName)
    ...
    switch (eventName) {
      case '_Challenge':
        return olderMatch.merge({
          ...
          pollID: logData.challengeID.toString(),
          commitEndDate: logData.commitEndDate.toString(),
          revealEndDate: logData.revealEndDate.toString(),
          votesFor: '',
          votesAgainst: '',
          votesTotal: '0',
          userVoteNumTokens: '',
      ...
      case '_ApplicationWhitelisted':
        if (olderMatch.get('userVoteChoice') === '1') {
          return olderMatch.merge({
            tokensToClaim: true,
            status: 'whitelisted',
            whitelistBlockTimestamp: txData.get('blockTimestamp').toString(),
          })
        }
        return olderMatch.merge({
          status: 'whitelisted',
      ...
      case '_ChallengeSucceeded':
        return olderMatch.merge({
          ...
          removedBlockTimestamp: txData.get('blockTimestamp').toString(),
        })
      case '_VoteCommitted':
        // prettier-ignore
        // Increment the total votes committed to this listing
        // w/ the numTokens committed during this particular event log
        return olderMatch.set('votesTotal', 
          BN(olderMatch.get('votesTotal')).add(BN(logData.numTokens)).toString()
        )
      case '_VoteRevealed':
        ...
        //   })
        // }
        console.log('line 203', 'logData:', logData)
        return olderMatch.merge({
          votesFor: BN(logData.votesFor).toString(),
          votesAgainst: BN(logData.votesAgainst).toString(),
        })
      case '_RewardClaimed':
        ...
        // if (logData.voter === this.user) {
        //   return olderMatch.set('tokensToClaim', fromJS(false))
        // }
        return olderMatch
      case '_PollCreated':
        return olderMatch.merge({
          status: 'challenged',
          pollID: logData.pollID.toString(),
        })
      default:
  ...
  replaceDuplicateListings(newListings, listings = {}) {
  ...
  findMatchingListing(logData, listings) {
    switch (logData._eventName) {
      ...
      case '_VoteRevealed':
        return listings.find((v, k) => {
          return v.get('pollID') === logData.pollID.toString()
        })
      case '_RewardClaimed':
        return listings.find((v, k) => {
          return v.get('pollID') === logData.challengeID.toString()
        })
      default:
        return false
    }
